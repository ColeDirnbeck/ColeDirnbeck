...
from bs4 import BeautifulSoup       # needed for the FinViz data scraper
import requests
...
class Stock(object):
    ...
    def load_financial_data(self):
        # loads financial data of the ticker from yahoo finance
        print(f"Loading financial data for {self.ticker}")
        self.yfin.load_latest_data()
        
    def calc_returns(self):
        # calculates daily returns of the ticker
    ...
    # grabs the Total Debt value of the ticker from the balance sheet tab on yahoo finance
    def get_total_debt(self):
        try:
            ...
            print(f"Failed to get total debt for {self.ticker}: {e}")       # print error if value isn't found
            return None

    # grabs the Free Cash Flow value of the ticker from the cashflow sheet tab on yahoo fiannce
    ...
    # grabs the Cash, Cash Equivalents, and Short Term Investments value of the ticker from the balance sheet tab on yahoo finance
    ...
    # grabs the Shares Issued value of the ticker from the balance sheet tab on yahoo finance
    ...
    # version number 2 to do get_beta (from yahoo finance)
    ...
    #         return result
    #     except Exception as e:
    #         print(f"Failed to get beta for {self.ticker}: {e}")             # print error if value isn't found
    #         return None

    

    # version number 1 to do get_beta (from FinViz)
    # found from https://medium.datadriveninvestor.com/scraping-live-stock-fundamental-ratios-news-and-more-with-python-a716329e0493
    def get_beta(self):
    ...
    # takes the beta value found on FinViz and converts it to a discount rate value (implemented from the table on the blog)
